...
export function PromptWorkflowPanel({ steps }: PromptWorkflowPanelProps) {
  return (
    <section className="rounded-[28px] border border-white/12 bg-white/8 p-5 shadow-[0_32px_80px_rgba(0,0,0,0.35)] backdrop-blur-2xl md:p-6">
      <div className="mb-5">
        <p className="text-xs uppercase tracking-[0.32em] text-cyan-100/60">Prompt Workflow</p>
        ...
        {steps.map((step) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs uppercase tracking-[0.28em] text-cyan-100/85">
                {step.id}
              </span>
              <h3 className="font-medium text-white">{step.title}</h3>
            </div>
            <p className="mt-3 leading-6 text-white/62">{step.purpose}</p>
            <pre className="mt-4 overflow-x-auto rounded-[18px] border border-white/10 bg-[#07111d]/85 p-4 text-xs leading-6 text-white/78">
